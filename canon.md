# Canon — 사막의 무명신

Hard facts. 작성된 모든 산문은 이 캐논과 모순되면 안 된다.
Phase 1 시드 항목들. Phase 2 (drafting) 에서 챕터별 추가.

---

## Geography

- 케르샤 신왕국 = 사막 가운데 강 하르라를 따라 형성. 남북 약 2,400리,
  동서 약 1,800리. (world.md)
- 도성 = 나르하트. 강 하르라의 가장 큰 굽이. 12 신전이 광장을 둘러
  쌈. 신왕궁 = 광장 북쪽, 가장 큰 피라미드형. (world.md)
- 외성 샤르 = 도성 외곽 빈민가. 약한 신탁자·노예 거주. (world.md)
- 카드라 사막 = 도성 서쪽. 사막 신후 영토. 옛 폐허 다수. (world.md)
- 테무르 강 상류 = 강 신후 영토. 비옥한 농경지. (world.md)
- 이쉬 산맥 = 동쪽 경계. 외방 부족 거주. (world.md)
- 검은 도시 자흐라 = 옛 봉인 사건 폐허. 카드라 사막 가장 깊은 곳.
  봉인의 근원. (world.md)
- 거리 — 도성→자흐라 낙타 14-20일. 도성→이쉬 낙타 10-12일.
  도성→카드라 변두리 낙타 3-5일. (world.md)

## Timeline (작품 시작 기준 역순)

| 시점 | 사건 | 출처 |
|---|---|---|
| -1,200년 | 잊힌 신 봉인 사건. 12명이 회의록을 남김. 자흐라가 그 폐허. | world.md |
| -약 60년 | 무한 출생 (작품 시작 시 60대) | characters.md |
| -50년 | 무한이 자흐라 인근까지 다녀와 침묵을 택함 | characters.md |
| -35년 | 신왕 아셀 출생 (작품 시작 시 35세) | characters.md |
| -22년 | 신왕 아셀 의식의 날 (13세). 표식이 너무 강해 의식 도중 기절. | characters.md |
| -17년 | 라이샤르 출생 (작품 시작 시 17세) | characters.md |
| -16년 | 셈한 출생 (작품 시작 시 16세) | characters.md |
| -14년 | 시샤 출생 (작품 시작 시 14세) | characters.md |
| -13년 | 하셈 출생 (작품 시작 시 13세) | characters.md |
| -12년 | 신왕 아셀 즉위 | characters.md |
| -8년 | 하셈 부모 사망 (전염병). 외성 노파가 양육 시작. | characters.md |
| -5년 | 추적자 에난이 무고한 부적절 신탁자를 처리함. 후에 이를 알게 됨. | characters.md |
| -4년 | 라이샤르 의식의 날. 지식 강한 신탁 → 의례 직후 도주. | characters.md |
| **0** | **작품 시작. 의식의 날 직전.** Arc 1 회 1. | outline.md |
| +1일 | 회 1-3: 의식의 날. 하셈 0 표식. | outline.md |
| +1주 | 회 8-9: 외성 노파 살해. | outline.md |
| +2-3주 | 회 14-30: 도서관 잠입, 봉인 회의록 발견. | outline.md |
| +1개월 | 회 40-50: Arc 1 클라이맥스. 약방 화재, 셈한 결단. | outline.md |
| **약 +2개월** | **Arc 2 시작.** 자흐라 향한 사막 횡단. | outline.md |

**연력 표기 규칙**: 사회 공식 연력은 신왕 즉위년 기준. 작중 명시는
"신왕 12년 봄" 식. 작품은 신왕 12년 봄에서 시작해 신왕 13년 봄까지
약 1년 시간 범위 (Arc 1-4 합산 추정).

## Magic System Rules (HARD)

- 의식 후 신탁은 변하지 않음. 단 잊힌 신 매개로는 변할 수 있음.
  (world.md HARD)
- 신탁 능력은 받은 신 영역에만 작동. (world.md HARD)
- 0 신탁자는 모든 신성 매개 작동 안 함. 단 태초신 매개는 표식
  시스템을 우회. (world.md HARD)
- 같은 신 신탁자 사이엔 본능적 연대·서열. (world.md HARD)
- 표식은 죽음 직전 흐려지며 이름과 함께 사라진다. (world.md HARD)
- **신탁 능력은 그 신의 영역적 환경에 의존.** 사막 신탁 = 모래·사막
  흙. 강 신탁 = 수로·우물·강. 바람 신탁 = 트인 공간·높은 곳. 의술
  신탁 = 약초·환자 곁. 환경 밖에선 능력 약화. (world.md HARD)

## Aura / 시각 시스템 (HARD)

- 표식 = 누구나 봄. 색·크기·위치로 신과 강도 식별. (world.md)
- 아우라 = 표식 주위 살아있는 색의 흐름. 평범한 사람 못 봄. 도용
  신은 자기 신탁자 아우라 일부 봄. 같은 신 사제단 훈련으로 일부.
  **0 신탁자 하셈만 모든 신 아우라 날것 그대로 봄.** (world.md)
- 12 도용 신탁 아우라엔 결이 어긋난 부분 있음 = 도용 시각 흔적.
  하셈만 봄. (world.md)
- 잊힌 신의 색 = 12 색 어디에도 속하지 않음. 작중 호명 "이름이
  없는 색", "어떤 색도 아닌 색". (world.md)
- E층 옛 핏줄 흔적자 = 받은 도용 신탁 색 옆에 이름 없는 색 잔향.
  본인 못 봄. 하셈만 봄. (world.md)
- 약한 신탁자 = 색이 거의 안 보일 정도로 흐림. (world.md)
- 죽음 직전 = 표식과 색이 함께 흐려짐. (world.md)

### 12 도용 신 색표

| 영역 | 색 |
|---|---|
| 태양 라이엔 | 진한 금 |
| 달 | 은청 |
| 강 | 짙은 청록 |
| 사막 | 황동·녹슨 적동 |
| 바람 | 옅은 은회 |
| 곡식 | 짙은 황갈 |
| 죽음 | 회흑 (재) |
| 전쟁 | 녹슨 적 (동의 적) |
| 풍요 | 짙은 녹 (포도 잎) |
| 지식 | 깊은 남청 (먹) |
| 의술 | 백연자 |
| 심판 | 흑자 |

## 잊힌 신 속삭임 비용 (HARD)

- **시각 비용 (단기)**: 속삭임 후 약 24시간 동안 다른 사람 아우라
  색이 흐려져 보임. 속삭임 빈번할수록 회복 늦음. (world.md)
- **기억 비용 (영구)**: 속삭임마다 자기 기억 한 조각 영구히 흐려짐.
  회 4 첫 속삭임 직후 부모 얼굴부터. Arc 4 자기 자신 흐려짐의
  누적. (world.md)
- 잊힌 신은 비용을 미리 말하지 않음. 거짓이 아닌 침묵. (world.md)

## Magic System Rules (SOFT)

- 두 신탁자가 같이 능력 사용 시 약한 공명. 같은 신끼리 더 큼.
  (world.md)
- 사막 깊은 곳·강 깊은 곳·옛 폐허에선 도용 신탁이 약해짐. 잊힌 신
  속삭임이 들리기 쉬움. (world.md)
- 12 도용 신은 서로 영역 견제. 다른 영역에서 능력 약해짐. (world.md)

## Character Facts

- 하셈 = 13세, 외성 샤르 출신 고아, 사회 유일의 0 신탁자, 옛 태초신
  핏줄 후손. 검은 손바닥 자국 = 옛 핏줄 표식. (characters.md)
- 하셈 부모 = 5세 때 전염병 사망. 다른 가족 양육 거부. (chars.md)
- 하셈 양육 = 외성 노파(이름 모름). 의식의 날 직후 사라짐(살해).
  (characters.md, outline Ch 8-9)
- 시샤 = 14세, 외성 샤르의 약한 바람 신탁자. 옛 핏줄 흔적자(E층)
  지만 본인 모름. (characters.md)
- 시샤 표식 = 손목 안쪽 작은 회색 소용돌이. (characters.md)
- 라이샤르 = 17세, 신탁 귀족 가문 셋째 아들, 지식 신 강한 신탁자,
  의식의 날 직후 도주. (characters.md)
- 라이샤르 표식 = 왼쪽 어깨 푸른 책 모양. (characters.md)
- 무한 = 60대, 의술 약한 신탁자, 외성 약방 주인, 옛 핏줄 흔적자
  자각자. 검은 손바닥 자국 보유. (characters.md)
- 무한 50년 전 자흐라 인근 다녀옴, 침묵 택함. (characters.md)
- 신왕 아셀 = 35세, 태양 신 라이엔 강한 신탁자, 즉위 12년차.
  (characters.md)
- 아셀 13세 의식의 날에 표식이 너무 강해 기절. (characters.md)
- 추적자 나르 = 사막 신 추적관, 30대, 광신, 가족 중 부적절 신탁자
  있었음. (characters.md)
- 추적자 에난 = 강 신 추적관, 40대, 회의자, 5년 전 무고한 처리
  뒤늦게 알게 됨. (characters.md)
- 셈한 = 16세, 라이샤르 형, 카드라 가문 차남, 사막 신 강한 신탁자.
  옛 핏줄 흡수된 길의 거울 인물. (characters.md)
- 자흐라의 그림자 = 옛 핏줄 후손, 잊힌 신 권능 일부를 사적 도용.
  주인공의 어둠 거울. (characters.md)
- 사라이 = 30대 의술 부사제, 자기 신 거짓 의심 시작. (chars.md)

## Political / Factional

- 신왕 1명 + 신후 11명 = 12 권력자. (world.md)
- 신왕은 태양 신 라이엔의 강한 신탁자만. (world.md)
- 신후 = 12신 각자의 영역 통치. 같은 가문 내 순환. (world.md)
- 신왕 vs 신후 견제 + 신후들 사이 영역 갈등이 정치 동력. (world.md)
- 12 신탁 귀족 가문. 각 가문은 12신 중 하나와 강한 연결. (world.md)
- 외부자 군 = A 약한 신탁자 / B 거부 도망자 / C 부적절 신탁자 /
  D 경계자 / E 옛 핏줄 흔적자. 5층 다층. (world.md)
- 외방 부족 (이쉬 산맥 너머) = 케르샤 신탁 시스템 미편입.
  (world.md)

### 12 도용 신 + 신왕·신후 명단

| 영역 | 신탁자 정점 | 작중 등장 인물 | 비고 |
|---|---|---|---|
| 태양 라이엔 | 신왕 아셀 | 안타고니스트 정점 | 정점, 정치·군 권력 집중 |
| 달 | 달 신후 | 미정 (Arc 2-3 등장) | 신왕과 자주 대립 |
| 강 | 강 신후 | 추적자 에난 | Arc 3 동맹 전향 후보 |
| 사막 | 사막 신후 | 추적자 나르 / 셈한 / 카드라 가문 | 광신 + 옛 핏줄 흡수 거울 |
| 바람 | 바람 신후 | 시샤 (약한 신탁) | 외성 친화 |
| 곡식 | 곡식 신후 | (강 신후와 영지 분쟁) | 외성 노파 흐릿한 신탁 |
| 죽음 | 죽음 신후 | 미정 | 가장 두려운 권력 |
| 전쟁 | 전쟁 신후 | 미정 | 신왕 직속 칼 |
| 풍요 | 풍요 신후 | 미정 | 사회 구조 기초 |
| 지식 | 지식 신후 | 라이샤르 (강한 신탁, 도주자) / 라이샤르 가문 | 가문 = 카드라 가문 (사막 신후) 셋째 가지 |
| 의술 | 의술 신후 | 무한 (약한, 외성) / 사라이 (부사제) | 가장 약한 권력, Arc 2 첫 동맹 |
| 심판 | 심판 신후 | 미정 | 신왕 다음의 형식 권위 |

**주의**: 라이샤르 가문은 카드라 가문의 분가 또는 별개 가문. world.md
와 characters.md 에서 "카드라 가문" 표기로 통일 (라이샤르·셈한 모두
카드라 가문 형제). Arc 2 spec 시 가문 구조 정밀화 필요 (debt D3).

## Cultural / Procedural

- 의식의 날 = 봄 첫 보름달 밤, 광장에서 12 동심원으로 청소년 정렬.
  (world.md)
- 표식 노출 의무 = 의례·관청·결혼 등 공적 영역. 평소엔 옷으로 가능.
  (world.md)
- 이름 = 가문명 + 신탁명 + 본명. 0 신탁자는 신탁명 없음. (world.md)
- 0 신탁자 의복 = 검은색 (사회적 사망의 표지). (world.md)
- 죽음의 의례 = 가족이 이름을 큰 소리로 불러 표식 보존 시도.
  (world.md)
- 이름 없는 노래 = 외방·사막 부족의 가사 없는 자장가. 도성 금지.
  (world.md)
- 검은 손바닥 자국 = 옛 핏줄 흔적, 어렸을 때 박힘, 보통 사라짐.
  안 사라지면 박해 대상. (world.md)

## Established In-Story (Arc 1 cycle 1 의 설정 — drafting 진입 후 갱신)

- 작품 시작일 = 의식의 날 (회 1).
- 회 1: 하셈이 외성에서 도성으로 이동. 노파가 손바닥 천을 다시 묶음.
  하셈이 광장 가는 길에 다른 청소년의 옅은 색 안개를 본다 (자기
  단독 시각 능력 첫 발화).
- 회 2: 마지막 신까지 호명 후 표식 안 새겨짐. 광장 침묵. 하셈이
  사제들 아우라의 결이 어긋난 부분을 처음 알아챔 (회 50 자각의
  plant).
- 회 3: 신왕 아셀의 명령 = "처리하라." 하셈이 사제 손목 표식 색이
  흐릿함을 봄. 시점 교체 직전 신왕의 진한 금빛도 결이 어긋남.
- 회 4: 하셈이 잊힌 신의 첫 속삭임 ("들리는가?") 들음. 직후 자기
  검은 손바닥 자국 주위에 이름 없는 색이 옅게 어른거림. 시각 비용
  발동 — 24시간 동안 다른 사람 색 흐려짐.
- 회 5: 노파가 자기 흐릿한 풍요 표식을 처음 보여줌. 하셈이 그 표식
  옆에 이름 없는 색 잔향을 봄 (E층 옛 핏줄 흔적의 시각 시그널).
- 회 6: 무한 약방. 무한이 손바닥 자국 보여줌. 무한의 의술 백연자
  옆에도 이름 없는 색 잔향.
- 회 7: 추적자 나르 약방 등장. 약방 흙·약초 가루 위에서 사막 신탁
  추적이 약화됨 (모래 환경 의존). 하셈을 못 알아봄.
- 회 8: 노파가 "약속" 의 존재 처음 언급 — 받은 자 모름.
- 회 9: 노파 사망. 시신의 표식 완전히 흐려짐. 하셈이 부모 얼굴을
  떠올리려다 흐릿함을 처음 자각 (속삭임 기억 비용).
- 회 10: 시샤가 하셈을 처음 숨겨줌. 하셈이 시샤 손목 회색 소용돌이
  옆에 이름 없는 색 잔향을 봄 (시샤 E층 시각 plant).
- 회 16: 셈한이 가문 정보망으로 라이샤르 도서상 출입 첫 포착
  (가문 그림자 추적 시작 — 회 49 정면 등장의 plant).
- 회 26: 라이샤르 가문 도서관에서 봉인 회의록 두루마리 발견.
- 회 29: 가문 안 신고 = 셈한 정보망 일원 (회 30 무한 지하실에서
  라이샤르가 추측, 회 49 확정).
- 회 30: 무한이 손바닥을 하셈에게 겹침. 하셈이 봉인의 밤 환영.
- 회 40: 약방 화재. 나르가 마침내 진입했으나 무한이 표식 흐리는
  의례로 사막 신탁 일시 정지. 무한 행방 불명 (사망은 Arc 2 확인).
- 회 45: 하셈의 첫 신성 매개 작동 (외성 외곽 모래 위 빛 흔적).
- 회 50: 외성 외곽 모래 언덕 = 처음으로 모래 환경, 나르 진짜 강도
  발동. 셈한이 같은 사막 신탁으로 나르를 막아내며 동생 살리기
  결단. 하셈이 자기 손바닥 자국 = 표식 자각, 자국 주위에 이름
  없는 색이 처음 또렷해짐. Lie "내가 부족해서" 첫 균열.

## Internal Consistency Constraints

1. 0 신탁자는 하셈 1명 (1,200년 만의 단일 사례). 다른 인물의 "선택
   받지 못한 자" 묘사 = 약한 신탁자(A) 또는 거부자(B). (world.md)
2. 잊힌 신 직접 감응 = 0 신탁자만. 옛 핏줄 흔적자(E) 는 본능적
   의심·꿈만, 직접 음 못 들음. (world.md)
3. 신왕은 태양 신 강한 신탁자. 표식 강도 상위 1% 이내. (world.md)
4. 결말의 "신 없는 세계" = 12 도용 신 + 잊힌 태초신 모두 사라짐.
   어떤 신탁도 작동 안 함. (world.md)
5. 잊힌 신 봉인 = 자흐라 가장 깊은 곳. 0 신탁자가 도달해야 풀림.
   (world.md)
6. 의식의 날 = 작품 시작. Arc 1 의 1-3회. (outline.md)
7. 잊힌 신의 호명 = 작중 절대 이름으로 부르지 않음. "그분", "이름
   없는 분", "잊힌 분", "처음의 분", "옛 분" 으로만. (world.md)
8. 잊힌 신은 거짓말하지 않음. 단 모든 진실을 한 번에 주지도 않음.
   (characters.md)
9. 회차 호흡 — 한 회 5,000-5,200자, 회당 진전 1-2개 + cliffhanger
   1개. (voice.md)
10. 시점 교체는 5-10회마다 1회. 회차 첫머리에 시점 인물 명시.
    (voice.md)
