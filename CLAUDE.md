...
### 할 것
1. **작품별 진입점 우선**. 프로젝트 루트에 `NOVEL-INDEX.md` 가 존재
   하면 그것이 그 작품의 진입점이다. 새 세션 시작 시 가장 먼저 읽고,
   같은 폴더의 `STATUS.md` 도 함께 본다 (현재 진행·다음 스텝).
2. **룰북을 읽고 직접 수행**. 작업 종류 → 읽을 문서:
   - 글쓰기/리비전: `voice.md` Part 1, `ANTI-SLOP.md`, `ANTI-PATTERNS.md`
   - 한국어 작품: `ANTI-SLOP-KO.md` (있으면)
   - 구조/플롯/인물 설계: `CRAFT.md`
   - 단계별 의도: `PIPELINE.md`, `program.md`
   - 작품 용어: `GLOSSARY.md` (있으면)
2. **5계층을 항상 의식**한다:
   ```
